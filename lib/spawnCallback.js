...
var spawn = major <= 4 ? require('./cross-spawn-7.0.3').spawn : require('cross-spawn').spawn;

module.exports = function spawnCallback(command, args, options, callback) {
  var localSpawn = options.spawn ? options.spawn : spawn;
  ...
  var cp = localSpawn(command, args, options);
